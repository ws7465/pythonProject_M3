...
#   other_words, которые содержат root_word или наоборот root_word содержит
#   одно из этих слов.
#   После вернуть список same_words в качестве результата своей работы.
#
# Пункты задачи:
#
#     Объявите функцию single_root_words и напишите в ней параметры
#       root_word и *other_words.
#     Создайте внутри функции пустой список same_words, который пополнится
#       нужными словами.
#     При помощи цикла for переберите предполагаемые подходящие слова.
#     Пропишите корректное, относительно задачи условие, при котором
#       добавляются слова в результирующий список same_words.
#     После цикла верните образованный функцией список same_words.
#     Вызовите функцию single_root_words и выведете на экран(консоль)
...
def single_root_words(root_word, *other_words): #
    same_words = [] #
    root_wordU = root_word.upper() #
    list_other_words = list(other_words) #
    for l in list_other_words : #
        L = l.upper() #
        if root_wordU.count(L) > 0:  #
            same_words.append(l)  #
        elif L.count(root_wordU) > 0 : #
            same_words.append(l) #
    return  same_words
#
#
